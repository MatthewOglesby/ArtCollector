...
Destructure so many things from the featured object

Render them into a template

Two of them, people and images, are likely to be arrays, so you'll need two maps to render them

Make sure to appropriately use react fragments or react fragment short syntax when you are pairing up the two spans per property
